...
if (sendNotifyBtn) {
    sendNotifyBtn.addEventListener("click", async () => {
        const type = notifyTargetType ? notifyTargetType.value : "all";
        const targetEmail = notifyTargetEmail ? notifyTargetEmail.value.trim().toLowerCase() : "";
        const message = notifyMessage ? notifyMessage.value.trim() : "";

        if (!message) {
            showNotification("يرجى كتابة نص الإشعار.");
            return;
        }

        if (type === "specific" && !targetEmail) {
            showNotification("يرجى إدخال إيميل المستخدم المستهدف.");
            return;
        }

        try {
            sendNotifyBtn.disabled = true;
            sendNotifyBtn.textContent = "جاري الإرسال...";

            const notifData = {
                message: message,
                target: type === "specific" ? targetEmail : "all",
                sentBy: currentAdmin.email || "أدمن",
                read: false,
                createdAt: serverTimestamp()
            };

            const notifRef = await addDoc(notificationsCollection, notifData);

            await writeLog({
                action: "send_notification",
                collectionName: "notifications",
                targetDocId: notifRef.id,
                oldData: null,
                newData: { ...notifData, createdAt: null },
            ...
            showNotification("حدث خطأ أثناء إرسال الإشعار.");
        } finally {
            sendNotifyBtn.disabled = false;
            sendNotifyBtn.textContent = "إرسال الإشعار";
        }
    });
}
